...
-(void) drawTraces :(NSMutableArray*) container{
   ...
   // NSLog(@"%d", idy);
    //int numberOfSineCurves = 30;
    //float cWidth = 1;
    float cWidth = ((float)width)/[[[(AppDelegate *)[[NSApplication sharedApplication] delegate] durationArray] objectAtIndex:idy] floatValue];
   ...
   // float cWidth = 0.5;
    ...
    for (int i=0; i < numberOfPoints; i++)
    {
       ...
       // [[NSColor whiteColor] set];
        ...
        [sinePath moveToPoint:NSMakePoint(i*cWidth,  height/2 - [[points objectAtIndex:i] floatValue]*cHeight*gain + height*idy + offset)];
